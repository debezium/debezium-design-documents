...
On the technical side, my primary language is Java and my stack spans Spring Boot, Kafka, Docker, Kubernetes, and cloud-native architecture. I've also worked with JavaScript, TypeScript, Node.js, Next.js, and Angular on the frontend, and I'm comfortable with SQL, AWS, CI/CD pipelines, and observability tooling like Prometheus, Grafana, Loki, and Tempo. Most of what's on that list isn't just theoretical - I've used it in projects I've actually shipped and in open source codebases I've contributed to.
...
This is the heart of the project. Once the host is ready and we have secure access to it, we need to actually run Debezium Server on it. The requirement specifies container-based runtimes - meaning Docker primarily.

Debezium Server is available as a Docker image. So deployment means:
...
The reason Docker is the right choice here is consistency. The same container runs identically whether the host is a bare metal server in someone's data center or a cloud VM in AWS. That's exactly what this project needs.
...
This is the layer that makes everything else composable. The UI, the CLI, or any automation script can talk to these endpoints without caring about what's happening underneath - whether that's an SSH connection, a Docker command, or anything else.
...
This project adds a second `EnvironmentController` implementation - a `HostEnvironmentController` - that handles deployments on bare-metal hosts and VMs via SSH + Docker. The Watcher's event-driven architecture doesn't need to change at all. The new controller plugs into the same event flow seamlessly.
...
For host-based deployment, the equivalent of that CRD will be a running Docker container on a remote machine. Instead of a `PipelineMapper` producing a Kubernetes resource, I'll build a `HostPipelineMapper` that produces an `application.properties` file and Docker commands, both delivered over SSH.

The pipeline model itself is already environment-agnostic. The platform allows users to define a source, a destination, and any data transformations, which are then composed into a deployable pipeline. Internally, the `Pipeline` view and `PipelineEntity` contain no mention of Kubernetes, Docker, or any environment-specific concept. The environment-specific translation is handled by separate components - today that's the `PipelineMapper` and `OperatorPipelineController` for Kubernetes.
...
**Credential Security:** SSH credentials must never be stored in plain text. I'll integrate with the platform's existing secrets management (which uses Kubernetes Secrets for the Kubernetes path). For host-based deployments, I'll store SSH key references - not raw keys - in the database, and the actual keys are loaded from a configurable secrets backend (environment variable, filesystem path, or a secrets manager like HashiCorp Vault via a pluggable `SecretProvider` interface). This way the database never holds sensitive material.
...
**What this is:** When a user registers a new host as a deployment target, the platform needs to validate that host and prepare it before any pipeline can be deployed there. This is the automated provisioning requirement.
...
**Check 2: Docker Installation Check** - Run `docker version --format '{{.Server.Version}}'`. Parse the output. If Docker is not installed, and if the host runs Ubuntu/Debian, remediate by running `apt-get install -y docker.io` over SSH. For RHEL/CentOS, use `yum install -y docker`. The OS is detected via `cat /etc/os-release`.

**Check 3: Docker Daemon Running Check** - Run `systemctl is-active docker`. If Docker is installed but not running, remediate with `sudo systemctl start docker && sudo systemctl enable docker`.

**Check 4: User Permissions Check** - Run `groups` and verify the current user is in the `docker` group. Without this, `docker run` commands will fail with permission denied. Remediate with `sudo usermod -aG docker $USER`.
...
**Check 6: Disk Space Check** - Debezium Server Docker images are ~500MB. Run `df -h /` and parse available space. If less than 2GB is available, warn the user.
...
##### Component 4: The Host Deployment Engine (Running Debezium Server via Docker)

**What this is:** This is the heart of the project. Once a host is provisioned and we have a valid SSH session, this component handles the actual deployment lifecycle - deploy, update, stop, remove - of a Debezium Server container.

**How I'll implement it:**

I'll build a `DockerDeploymentEngine` that translates pipeline definitions into Docker commands executed over SSH.

**Deploying a Pipeline:**

The entry point is the `PipelineController.deploy(PipelineFlat pipeline)` method, which receives a flattened view of the pipeline (including its source, destination, and transform configurations all denormalized). The `HostTarget` is resolved from the pipeline's associated `HostDeployment` record. Here's what happens step by step:
...
**Step 2** - Upload the configuration to the remote host. I'll use SFTP (which MINA SSHD also provides) to write this file to a dedicated directory on the remote host, e.g., `/opt/debezium/pipelines/{pipelineId}/application.properties`. This directory is created if it doesn't exist.

**Step 3** - Pull the Debezium Server image. Run `docker pull quay.io/debezium/server:{version}` over SSH. Parse the output to confirm success.

**Step 4** - Start the container. Run:
...
**Step 5** - Verify the container started. Run `docker inspect debezium-pipeline-{pipelineId} --format '{{.State.Running}}'`. If it returns `true`, deployment succeeded. If the container immediately exited, fetch the logs with `docker logs debezium-pipeline-{pipelineId} --tail 50` and include them in the error response so the user has actionable information.
...
- If a container already exists → update flow:
  - → Generate the new `application.properties`.
  - → Upload it via SFTP, overwriting the existing file.
  - → Restart the container: `docker restart debezium-pipeline-{pipelineId}`.
  - → Verify it's running again.
  - → Update the `configHash` in the `HostDeployment` record.
...
- Remove: `docker rm debezium-pipeline-{pipelineId}` followed by `rm -rf /opt/debezium/pipelines/{pipelineId}` to clean up config files.

**Status Polling:**

The existing Watcher component does NOT poll deployment status - it uses an event-driven outbox pattern to trigger deployments. For Kubernetes, status is queried on-demand via the `KubernetesClient` when a user requests it through the API.

For host-based deployments, on-demand status isn't sufficient because there's no Kubernetes-style watch mechanism. I'll introduce a new `HostDeploymentStatusPoller` as a separate `@Scheduled` component (a Quarkus scheduled bean) that periodically checks the state of all active host deployments:
...
public class HostDeploymentStatusPoller {
    ...
    void pollHostDeployments() {
        // For each active HostDeployment, SSH in and run:
        // docker inspect debezium-pipeline-{pipelineId}
        // --format '{{.State.Status}}|{{.State.StartedAt}}|{{.State.ExitCode}}'
        // Parse output → update HostDeployment.deploymentStatus in DB
    }
}
```

This gives us `RUNNING`, `STOPPED`, `EXITED`, and `NOT_FOUND` states. This is a new architectural component - the existing platform doesn't have periodic polling - but it's necessary because SSH-based deployments don't support the event-driven pattern that Kubernetes provides natively.

Additionally, the `configHash` stored in the `HostDeployment` entity is compared on each poll cycle against the current pipeline configuration hash. If they differ, the deployment is flagged as `CONFIG_DRIFT`, which the UI can surface to the user. This drift detection is a new capability that even the Kubernetes path doesn't have today.

---

##### Component 5: The REST API (Host Management + Pipeline Extensions)

**What this is:** The existing `PipelineResource` already handles all pipeline CRUD operations at `/api/pipelines` - creating a pipeline automatically triggers deployment through the outbox event flow. I don't need a separate deployment REST endpoint because the platform's architecture is: create pipeline → outbox event → Watcher → PipelineConsumer → `HostPipelineController.deploy()`. This happens automatically.

What I do need to add are two things:
1. a new `HostTargetResource` for managing remote hosts as deployment targets, and
2. status and log sub-endpoints on the existing pipeline path for host-based deployments.
...
The existing `PipelineResource` already provides `GET /api/pipelines/{id}/logs` which delegates to the `EnvironmentController`. For host deployments, the `HostPipelineController.logReader()` method will provide an SSH-based `LogReader` that fetches Docker container logs remotely. No new endpoint is needed - the existing endpoint automatically uses the correct implementation through the `EnvironmentController` routing.
...
A deployment system that touches live infrastructure - SSH connections, Docker daemons, remote filesystems - is exactly the kind of system where an untested change can silently break a production pipeline. For that reason, testing is not an afterthought in this project. It is a first-class deliverable, designed alongside the implementation, not bolted on at the end.

My testing strategy is structured into three layers: unit tests that run instantly with no external dependencies, integration tests that validate real SSH and Docker behavior using isolated containers, and API-level tests that verify the REST endpoints behave correctly from end to end. Each layer has a distinct job, and together they create a safety net that covers every critical path in the system.

**Coverage Goals and Tooling:**

I'll introduce [JaCoCo](https://www.jacoco.org/jacoco/) for measuring code coverage, integrated with Maven so that coverage reports are generated automatically on every build. The existing project does not currently use JaCoCo, so this would be a new addition to the build - one that benefits the entire project, not just the host-deployment module.

The target is 80% line coverage across the `environment` package (where the deployment logic lives) and 90% line coverage for the config generation and command building utilities, which are pure functions with no side effects and have no excuse for less than complete coverage.
...
Test doubles - mocks and stubs - will be used exclusively for external systems that Testcontainers cannot realistically replicate, such as the secrets manager. All other external interactions (SSH, SFTP, Docker) will be tested against real systems via Testcontainers, because mocking an SSH client tells you nothing about whether your MINA SSHD configuration is actually correct.

**What the Test Suite Proves:**

By the end of the project, the test suite will prove four things that matter to the Debezium:

First, that the new host-based deployment path works correctly in isolation - you can deploy, update, and remove a Debezium Server container on a remote host via SSH and Docker without any manual intervention.
...
Fourth, that the system fails gracefully - a refused SSH connection, a missing Docker installation, a container that crashes immediately after starting - all of these produce clear, actionable error messages through the API rather than unhandled exceptions that leave the system in an unknown state.
...
**Why Docker over standalone Debezium Server JAR?** Debezium Server is available as a configurable, ready-to-use application. Using its container image guarantees that the same exact runtime runs identically whether the host is a bare metal server in a data center or a cloud VM. [Debezium](https://debezium.io/) It also makes cleanup trivial - removing a container removes everything.
...
**Why the `configHash` approach for drift detection?** Rather than polling Docker logs continuously, computing a SHA-256 of the applied configuration and comparing it on each `HostDeploymentStatusPoller` cycle is lightweight and reliable. It catches the case where someone manually changes a config file on the server directly - something that would otherwise be invisible to the platform. Note that the existing Watcher does not do periodic polling (it's event-driven via the outbox pattern), so this polling mechanism is a new component specific to host-based deployments, introduced because bare-metal hosts don't support the event-driven watch mechanisms that Kubernetes provides natively.
...
I will write weekly blog posts documenting my progress, technical learnings, and implementation decisions at https://medium.com/@kumardivyanshu118 - covering topics like SSH automation in Java, Docker lifecycle management, and how the Debezium Platform architecture works under the hood. These blogs won't just be progress updates - I want them to be genuinely useful for anyone who wants to understand this project or contribute to it later.
...
**Continuous Maintenance:** Real deployments surface edge cases that a controlled test environment never does. I'll actively monitor issues raised against the host-based deployment feature - whether it's an SSH authentication edge case on a specific Linux distro, a Docker version incompatibility, or an unexpected failure during Debezium Server provisioning on a bare metal setup. I'll stay responsive and treat these as part of my responsibility, not someone else's problem.
