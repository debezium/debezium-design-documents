...
Unlike relational databases that store data in tables with rows and columns, Neo4j is a [graph database](https://neo4j.com/docs/getting-started/appendix/graphdb-concepts/) that stores data as **nodes** and **relationships** (edges) forming a graph structure.
The **Nodes** represent entities (e.g., a person, a movie). A node can have zero or more **labels** that classify it (e.g., `Person`, `Actor`).
The **Relationships** connect two nodes and always have a direction and exactly one **type** (e.g., `ACTED_IN`, `DIRECTED`).
The **Properties** are key-value pairs stored on both nodes and relationships (e.g., `name: "Alice"`, `born: 1970`) and  are dynamically typed: the same property name can hold different types across nodes with the same label.

In Neo4J  the schema is optional. In fact it does not require a schema to be defined before inserting data, so two nodes with the same label can have completely different sets of properties. 
This is a fundamental difference from relational databases where every row in a table has the same columns. 
Neo4j does offer optional [constraints](https://neo4j.com/docs/cypher-manual/current/constraints/syntax/) (key, uniqueness, existence, property type) that can enforce rules when needed, but they are not required.

Example graph:

```
(:Person {name: "Kevin Bacon", born: 1958})-[:ACTED_IN {role: "Jack Swigert"}]->(:Movie {title: "Apollo 13"})
```

This creates two nodes (`Person` and `Movie`) connected by an `ACTED_IN` relationship with a `role` property.

These concepts map to the connector's design as follows:

| Neo4j concept | Connector equivalent |
|:---|:---|
| Node label | Kafka topic (e.g., `prefix.db.node.Person`) |
| Relationship type | Kafka topic (e.g., `prefix.db.relationship.ACTED_IN`) |
| Properties | Fields in the Debezium event value |
| Key constraint | Event key fields |
| `elementId` | Internal node/relationship identifier (included in events as metadata) |
...
* Support initial snapshots via [Cypher](https://neo4j.com/docs/cypher-manual/current/introduction/) (Neo4j's declarative graph query language) queries for captured labels and relationship types.
 ...
 * Neo4j Enterprise Edition 5.13+ (self-managed) or [Neo4j AuraDB](https://neo4j.com/docs/aura/) Business Critical / Virtual Dedicated Cloud (Neo4j's fully managed cloud offering).
* CDC must be enabled on the target database. On self-managed instances, set [`txLogEnrichment`](https://neo4j.com/docs/cdc/current/get-started/self-managed/) to `DIFF` or `FULL` via [`ALTER DATABASE`](https://neo4j.com/docs/operations-manual/current/database-administration/standard-databases/alter-databases/). On AuraDB, enable CDC via the [Aura console UI](https://neo4j.com/docs/cdc/current/get-started/aura/) ("Edit CDC Mode" option).
...
* [Transaction log retention](https://neo4j.com/docs/operations-manual/current/database-internals/transaction-logs/) must be configured (`db.tx_log.rotation.retention_policy`) to cover at least the maximum expected connector downtime. On AuraDB, log retention is fixed and not user-configurable.

### AuraDB compatibility

The connector communicates with Neo4j exclusively via the Bolt protocol and Cypher procedures, so it is fully compatible with [AuraDB](https://neo4j.com/docs/cdc/current/get-started/aura/) Business Critical and Virtual Dedicated Cloud tiers. 
CDC is **not available** on AuraDB Free or Professional tiers.

AuraDB-specific considerations:

* **CDC is disabled automatically** when an instance is paused, cloned, or restored from a snapshot. 
    Previously generated change identifiers become invalid. After resuming, the connector will need to reset offsets and perform a new snapshot.
* **Transaction log retention is fixed** and not user-configurable on AuraDB. 
     The retention window depends on the workload and will fluctuate over time. If the connector is offline for too long, older changes may be pruned before it reconnects.
* **CDC mode is configured via the Aura console UI**, not via `ALTER DATABASE`. The connector cannot verify or set the mode programmatically.
...
Neo4j's own [Kafka connector](https://neo4j.com/docs/kafka/current/) also uses polling, as does the [Neo4j GraphQL subscriptions](https://neo4j.com/docs/graphql/current/subscriptions/customize-cdc/) layer (which polls every 1 second by default).
